...
size_t ariel::Graph::getNumEdges() const { return numEdges; }
...
string ariel::Graph::printGraph() const {
    ...
    for (size_t i = 0; i < numVertices; i++) {
        ans.append("[");
        for (size_t j = 0; j < numVertices; j++) {
            if (adjMatrix[i][j] == INF) { ans.append("X"); }
            else if (i == j) { ans.append("-"); }
            else { ans.append(to_string(adjMatrix[i][j])); }
            if (j != numVertices-1) { ans.append(", "); }
            else {ans.append("]\n"); }
        }
    }
    return ans;
